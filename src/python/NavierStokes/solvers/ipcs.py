...
class Solver:
    ...
    def __init__(self, solverType='ipcs'):
        """
            Define the solver
            
            Usage
            -----
                Solver = Solver(solverType='ipcs')
                
            Parameters
            ----------
                solverType  :: the name of the solver.
                               (type: str; shape: single word) [optional]
                              
            Returns
            -------
                Solver      :: the solver with solver functions.
                               (type: classobj; shape: single)
        """

        # Solver name
        self.name = solverType # 'ipcs'
    

        
    def initialize(self, mesh, nu, dt, f):
        """       
        ...
        self.DG = dolfin.FunctionSpace(mesh, "DG", 0)
        ...
        self.n  = dolfin.FacetNormal(mesh) #TODO: will this cause problem?????????
        ...
        self.F1 = (1/self.k)*dolfin.inner(self.v, self.u - self.u0)*dolfin.dx \
                    ...
                    - dolfin.inner(self.v, f)*dolfin.dx
    ...
    def solve(self, bcu, bcp):
        ...
        if len(bcp) == 0: dolfin.normalize(b)
        [bc.apply(self.A2, b) for bc in bcp]
        dolfin.solve(self.A2, self.p1.vector(), b)
        if len(bcp) == 0: dolfin.normalize(self.p1.vector())

        # Velocity correction
        b = dolfin.assemble(self.L3)
        [bc.apply(self.A3, b) for bc in bcu]
