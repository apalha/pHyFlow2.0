...
#    :License:       GNU GPL version 3 or any later version   
    ...
    def __init__(self,vortex, navierStokes, interpolationRegions,
        ...
        self.__set_variables('vortex',vortex) # vortex-blobs class
        self.__set_variables('navierStokes', navierStokes) # panel class
        ...
        for i,subDomainID in enumerate(self.navierStokes.subDomainKeys):
                
            # Make references to probe grid parameters
            probeGridParams = self.navierStokes[subDomainID].probeGridParams
            ...
            if _numpy.abs(hGrid[0]-self.vortex.blobs.h) > _numpy.spacing(1):
                print "NS sub-domain : %s, hGrid : %g, hBlob : %g" % (subDomainID,hGrid[0], self.vortex.blobs.h)
    ...
    def evolve(self,cmGlobalNew,thetaLocalNew,cmDotGlobalNew,thetaDotLocalNew):
        ...
        * Note: moving panels not implemented yet 
        ...
        self.vortex.evolve() # TODO: panel position update not implemented !
        #----------------------------------------------------------------------                     
        ...
        self.__multiStep_navierStokes(cmGlobalNew,thetaLocalNew,cmDotGlobalNew,thetaDotLocalNew)
        #self.__singleStep_navierStokes(cmGlobalNew,thetaLocalNew,cmDotGlobalNew,thetaDotLocalNew)
    ...
    def __correct_blobStrength(self):
        ...
        self.__totalCirculationBeforeRemove = self.vortex.blobs.g.sum()
        ...
        self.vortex.blobs.addBlobs(_numpy.concatenate(xBlobNewList),
                                   _numpy.concatenate(yBlobNewList),
                                   _numpy.concatenate(gBlobNewList))
        
        # Check the circulation after we add
        self.__totalCirculationAfterAdd = self.vortex.blobs.g.sum()
    ...
    def __remove_old_blobs(self):
        ...
        for subDomainID in self.navierStokes.subDomainKeys:
            ...
            xBlob = self.vortex.blobs.x
            yBlob = self.vortex.blobs.y
            ...
            self.__totalCirculationRemoved.append(_numpy.sum(self.vortex.blobs.g[iBoundingBox[iInside]]))
        
            if iBoundingBox[iInside].shape[0] !=0:
                # Remove old blobs inside polygon
                self.vortex.blobs.removeBlobs(iBoundingBox[iInside])
    ...
    def __multiStep_navierStokes(self,cmGlobalNew,thetaLocalNew,cmDotGlobalNew,thetaDotLocalNew):
        r"""
        ...
        * Note: navier-stokes motion not implemeneted !
        ...
        # IMPORTANT : at t_{n+1}
        x,y = self.navierStokes.getBoundaryCoordinates() # we assume that the coordinates are at t_{n+1}
        #----------------------------------------------------------------------                         
        
        #----------------------------------------------------------------------                         
        # Determine the dirichlet velocity boundary condition at t_{n+1}
        vxNew, vyNew = self.vortex.evaluateVelocity(x.copy(),y.copy())
        #----------------------------------------------------------------------
        ...
        for k in range(1,self.nEulerianSubSteps+1):
            ...
            #self.navierStokes.evolve(vxyEulerianBoundary_subStep[0], vxyEulerianBoundary_subStep[1],
            #                         cmGlobalNew,thetaLocalNew,cmDotGlobalNew,thetaDotLocalNew) # TODO: moving ns domain not implemented !
            self.navierStokes.evolve(vx_substep, vy_substep,
                                     cmGlobalNew,thetaLocalNew,cmDotGlobalNew,thetaDotLocalNew) # TODO: moving ns domain not implemented !
    ...
    def __singleStep_navierStokes(self,cmGlobalNew,thetaLocalNew,cmDotGlobalNew,thetaDotLocalNew):
        ...
        * Note: navier-stokes motion not implemeneted !
        ...
        #----------------------------------------------------------------------                         
        # Determine the dirichlet velocity boundary condition at t_{n+1}
        vxyEulerianBoundary = self.vortex.evaluateVelocity(x.copy(),y.copy())
        #----------------------------------------------------------------------                        
        
        #----------------------------------------------------------------------                           
        #  Evolve the navier-stokes to the next step
        self.navierStokes.evolve(vxyEulerianBoundary[0], vxyEulerianBoundary[1],
                                 cmGlobalNew,thetaLocalNew,cmDotGlobalNew,thetaDotLocalNew) # TODO: moving ns domain not implemented !
    ...
    def __set_eulerian_deltaT(self):
        ...
        deltaTE = self.navierStokes.deltaT
        
        # Determine the lagrangina time-step size
        deltaTL = self.vortex.deltaT
        ...
        self.navierStokes.deltaT = deltaTL/self.nEulerianSubSteps
    ...
    def __interpolate_strength_from_ns(self,xBlobNewList,yBlobNewList):
        ...
        for i,subDomainID in enumerate(self.navierStokes.subDomainKeys):
            ...
            cmGlobal = self.navierStokes[subDomainID].cmGlobal
            thetaLocal = self.navierStokes[subDomainID].thetaLocal
            ...
            wGrid = self.navierStokes[subDomainID].getVorticity().copy()
    ...
    def __scipyInterpolate_strength_from_ns(self,xBlobNewList,yBlobNewList):
        ...
        for i,subDomainID in enumerate(self.navierStokes.subDomainKeys):
            ...
            solver = self.navierStokes.subDomains['NSDomain']._NavierStokes__solver
            ...
            gBlobNew = wBlobNew * (self.vortex.blobs.h**2)
    ...
    def __get_eulerian_initialBoundaryVelocity(self):
        ...
        vx, vy = self.vortex.evaluateVelocity(x.copy(),y.copy())
    ...
    def __set_variables(self,varName,var):
        ...
        if varName == 'navierStokes':
            if str(type(var)) != "<class '__main__.MultiNavierStokes'>":
        ...
        # Check and set interpolationRegion parameters
            ...
            for key in self.navierStokes.subDomainKeys:
