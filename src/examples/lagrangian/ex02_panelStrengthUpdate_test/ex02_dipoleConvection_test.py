...
blobs_varying = pHyFlow.blobs.Blobs(wField,vInf,nu,deltaTc,h,overlap,
                                    blobControlParams=blobControlParams)                                   
#------------------------------------------------------------------------------
...
panels_varying = pHyFlow.panels.Panels(geometries=geometries)
#------------------------------------------------------------------------------                                   
...
lagrangian = pHyFlow.lagrangian.LagrangianSolver(blobs,panels,
                                                 couplingParams={'panelStrengthUpdate':'constant'})

lagrangian_varyingStrength = pHyFlow.lagrangian.LagrangianSolver(blobs_varying,panels_varying,
...
lagrangian.panels.solve(vInf[0],vInf[1])
vx,vy = lagrangian.panels.evaluateVelocity(x.flatten(),y.flatten())
...
py.scatter(lagrangian.blobs.x,lagrangian.blobs.y,c='r',edgecolor='none',label='blobs [constant]')
py.scatter(lagrangian_varyingStrength.blobs.x,lagrangian_varyingStrength.blobs.y,c='k',edgecolor='none',label='blobs [varying]')
[py.plot(x,y,k+'-',label=geoName) for x,y,geoName,k in zip(lagrangian.panels.xyPanelGlobal[0],
                                                           lagrangian.panels.xyPanelGlobal[1],
                                                           lagrangian.panels.geometryKeys,['b','g','k'])]
py.legend()                                                            
py.grid(True)
py.axis('scaled')
py.axis([-4,4,-2,2])
...
for i in xrange(2000):  
    ...
    print i
    lagrangian.evolve()
    lagrangian_varyingStrength.evolve()
