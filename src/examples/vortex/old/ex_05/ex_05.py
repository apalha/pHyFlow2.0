"""
    ...
    ex_04
        ...
        [-10, 10] x [-10, 10] with associated circulation given by:
            w = gamma*(exp(-(((x-pVortex(1)).^2) + ((y-pVortex(2)).^2))/(2*epsilon*epsilon)))/(2*pi*epsilon*epsilon)
            
            w(x,y) = 50.0 * exp(-((x^2)+(y-1.0)^2)/(pi/24.0))/(pi/24.0) -
                     -50.0 * exp(-((x^2)+(y+1.0)^2)/(pi/24.0))/(pi/24.0)
            
        with:
            
            deltaX = deltaY = 20.0/nBlobs
...
nBlobs = numpy.array([512,512])         # the number of vortex blobs in x and y
...
xBoundsDomain = numpy.array([-10.0,10.0]) # define the bounds for the domain
yBoundsDomain = numpy.array([-10.0,10.0]) # define the bounds for the domain

xBoundsPlot = numpy.array([-10.0,30.0]) # define the bounds for the plot
yBoundsPlot = numpy.array([-10.0,10.0]) # define the bounds for the plot

nPlotPoints = [128,64]

def wExact(x,y): return (-50.0 * numpy.exp(-(((x+0.0)**2)+(y+1.0)**2)/(numpy.pi/24.0))/(numpy.pi*numpy.pi/24.0)\
                         +50.0 * numpy.exp(-(((x+0.0)**2)+(y-1.0)**2)/(numpy.pi/24.0))/(numpy.pi*numpy.pi/24.0))
#=============================================================================
...
for tStep in range(1,tSteps+1):
    ...
    start = time.time()
    xB,yB,wB = pHyFlow.vortex.evolution.convection(dt,xB,yB,wB,sigma,k=k,kernel=pHyFlow.options.GAUSS_KERNEL,\
                                                   hardware=pHyFlow.options.GPU_HARDWARE,method=pHyFlow.options.FMM_METHOD,\
