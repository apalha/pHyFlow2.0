...
nBlobs = numpy.array([500,500])         # the number of vortex blobs in x and y
...
gthreshold = 10**-9 # the minimum value of circulation to consider
gmaxthreshold = 10**-9 # the maximum value of total circulation variation to consider
...
xBoundsPlot = numpy.array([-0.5,0.5]) # define the bounds for the plot
yBoundsPlot = numpy.array([-0.5,0.5]) # define the bounds for the plot
...
def wExact(x,y): return (1. * numpy.exp(-(((x)**2)+(y+0.1)**2)/(C0))/(numpy.pi*C0)\
                         +1.0 * numpy.exp(-(((x)**2)+(y+0.03)**2)/(C0))/(numpy.pi*C0)\
                         -1.0 * numpy.exp(-(((x)**2)+(y-0.03)**2)/(C0))/(numpy.pi*C0)\
                         -1.0 * numpy.exp(-(((x)**2)+(y-0.1)**2)/(C0))/(numpy.pi*C0)\
                         )
...
for tStep in range(1,tSteps+1):
    ...
    start = time.time()
    xB,yB,wB = pHyFlow.vortex.evolution.convection(dt,xB,yB,wB,sigma,k=k,kernel=pHyFlow.options.GAUSS_KERNEL,vInf=[2.0,0.0],\
