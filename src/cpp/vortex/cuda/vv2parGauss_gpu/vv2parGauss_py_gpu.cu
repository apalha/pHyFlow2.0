...
#include "vv2parGauss_device.cu"

/*--------------- Python callable function defintions -------------------*/
extern "C" static PyObject *vv2parGauss_gpu(PyObject *self, PyObject *args);
extern "C" void init_vv2parGauss_gpu();

/*----------------   C utility function definitions ---------------------*/


/*---------------- cuda utility function definitions --------------------*/
__global__ void vv2par_kernel(void *cxBlob_gpu_ondevice, void *cyBlob_gpu_ondevice, void *cwBlob_gpu_ondevice, void *cxTarget_gpu_ondevice, void *cyTarget_gpu_ondevice,void *cvelocities_gpu_ondevice);
...
static char vv2parGauss_gpu_docs[] =
"vv2parGauss_gpu computes induced velocities of particle distribution for a\n"
"Gaussian kernel using the GPU.\n"
"\n"
"Usage\n"
"-----\n"
"	vv2parGauss_gpu(xBlob,yBlob,wBlob,xTarget,yTarget,ksigmasqr,blocksize)\n"
"\n"
"Parameters\n"
"----------\n"
...
static PyMethodDef _vv2parGauss_gpu[] = {
   {"vv2parGauss_gpu", vv2parGauss_gpu, METH_VARARGS,vv2parGauss_gpu_docs},
   {NULL} /*Sentiner - marks the end of this Python structure*/
};

/*--------------Initialize the vv2par functions -------------------------*/
void init_vv2parGauss_gpu(){
   (void) Py_InitModule("_vv2parGauss_gpu", _vv2parGauss_gpu);
   import_array(); /* Must be present for Numpy */
}

/*-------------------- Python wrapping function - HOST -----------------------*/
static PyObject *vv2parGauss_gpu(PyObject *self, PyObject *args){
// Python wrapping function, pass data between Python and C and call CUDA Host Code.
  ...
  PyArrayObject *vx;
  PyArrayObject *vy;
  double *cxBlob,*cyBlob,*cwBlob,*cxTarget,*cyTarget, *cvx, *cvy;
  void *cxBlob_gpu, *cyBlob_gpu, *cwBlob_gpu, *cxTarget_gpu, *cyTarget_gpu, *cvx_gpu, *cvy_gpu;
  double myeps = 1e-12;
  double ksigmasqr;
  ...
  dims[1] = 2; // the 2 is the number of components of the velocity
  ...
  cvy = (double*)vy->data;
    ...
    cudaMemcpyToSymbol(inv_2pi_gpu, &inv_2pi,      sizeof(inv_2pi),      0, cudaMemcpyHostToDevice);
    ...
    cudaMalloc(&cvx_gpu, SIZE_IND);
    cudaMalloc(&cvy_gpu, SIZE_IND);
    ...
    vv2par_kernel <<<grid, threads, Sharedmemsize>>> (cxBlob_gpu, cyBlob_gpu, cwBlob_gpu, cxTarget_gpu, cyTarget_gpu, cvx_gpu, cvy_gpu);

    // Copy induction array from device to host memory
    cudaMemcpy(cvx, cvx_gpu, SIZE_IND, cudaMemcpyDeviceToHost);
    cudaMemcpy(cvy, cvy_gpu, SIZE_IND, cudaMemcpyDeviceToHost);
    ...
    // return the Python arrays with induced velocities as a python tuple
