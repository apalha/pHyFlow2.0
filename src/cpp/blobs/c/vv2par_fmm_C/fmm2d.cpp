...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
  ...
  // parse property/value-pairs in an automatic fashion
  for (int i = 2+eval_syntax; i < nrhs; i += 2) {
    PROPERTY prop;
    char str[15];

    // determine property
    if (!mxIsChar(prhs[i]))
      mexErrMsgTxt("Expecting a property name.");
    if (mxGetString(prhs[i],str,15) != 0 ||
        (prop = getprop(str)) == ERR_PROPERTY)
    {
      char errstr[100];
      strcpy(errstr,"Unknown property: ");
      strncat(errstr,str,80);
      mexErrMsgTxt(errstr);
    }

    // get its value
    if (prop != PANEL)
      // note: no getPANEL() in the list getval[], panels are handled below
      getval[prop](val[prop],prhs[i+1]);
    else {
      /* Get panel input, which is a cell-vector on the form
         {ZP,MP,...}, where ZP and MP are 2-by-NP complex matrices and
         where property/value-pairs specific to panels are
         understood. */
      int npanelrhs;

      if (!mxIsCell(prhs[i+1]))
        mexErrMsgTxt("Expecting a cell-vector as option 'panel'.");
      if ((npanelrhs = mxGetNumberOfElements(prhs[i+1])) < 2)
        mexErrMsgTxt("At least two elements required in option 'panel'.");
      if (npanelrhs&1)
        mexErrMsgTxt("Expecting panel property/value pairs.");

      const mxArray *ZP = mxGetCell(prhs[i+1],0);
      const mxArray *MP = mxGetCell(prhs[i+1],1);
      if (!ISDOUBLEMATRIX(ZP) || !ISDOUBLEMATRIX(MP) ||
          mxGetM(ZP) != 2 || mxGetM(MP) != 2 ||
          mxGetN(ZP) != mxGetN(MP))
        mexErrMsgTxt("The two first elements of option 'panel' must be "
                     "matching double matrices with two rows.");

      // allocate panels...
      Npanel = mxGetN(ZP);
      panels = (panel *)mxMalloc(Npanel*sizeof(panel));

      // ...and the data that will go into them
      int *smoothvec = (int *)mxMalloc(Npanel*sizeof(int));
      double *cutoffvec = (double *)mxMalloc(Npanel*sizeof(double));
      mxLogical *dirvec = (mxLogical *)mxMalloc(Npanel*sizeof(mxLogical));
      ...
      static const getpanelvalFun getpanelval[] =
        {&getpanelCUTOFF,&getpanelDIR,&getpanelSMOOTH};
      void *panelval[] = {cutoffvec,dirvec,smoothvec};

      // parse panel property/value-pairs
      for (int j = 2; j < npanelrhs-1; j += 2) {
        int prop;
        char str[10];
        const mxArray *Prop = mxGetCell(prhs[i+1],j);
        const mxArray *Val = mxGetCell(prhs[i+1],j+1);

        // determine property
        if (!mxIsChar(Prop))
          mexErrMsgTxt("Expecting a panel property name.");
        if (mxGetString(Prop,str,10) != 0 ||
            (prop = getpanelprop(str)) == ERR_PROPERTY)
          mexErrMsgTxt("Unknown panel property.");

        // get its value
        getpanelval[prop](Npanel,panelval[prop],Val);
      }

      // insert values and deallocate
      readpanels(panels,mxGetPr(ZP),mxGetPi(ZP),mxGetPr(MP),mxGetPi(MP),
                 dirvec,smoothvec,cutoffvec,Npanel);
      mxFree(dirvec);
      mxFree(cutoffvec);
      mxFree(smoothvec);
    }
...
#ifdef CHANNELPOT
  if(channelheight>0&&Npanel>0) { //create mirror panels
    panels = (panel *)mxRealloc(panels,Npanel*3*sizeof(panel));
    replicatepanelpositions(panels,Npanel,channelheight);
  }
#endif
#if !defined(CHANNELPOT) || defined(CUDASUPPORT)
  if(channelheight>0)
    mexErrMsgTxt("channelpot not supported, please recompile for cpu with channel support");
#endif
