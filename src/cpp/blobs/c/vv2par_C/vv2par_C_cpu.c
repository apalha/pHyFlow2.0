...
static PyObject *vv2par_C_cpu(PyObject *self, PyObject *args);
void init_vv2par_C_cpu();

/*----------------   C utility function definitions ---------------------*/

// define the doctrings for the functions so that help is available in Python
static char vv2par_C_cpu_docs[] =
"vv2par_C_cpu computes induced velocities of particle distribution for cutoff kernel.\n"
"\n"
"Usage\n"
"-----\n"
"	vv2parGauss_C_cpu(xBlob,yBlob,wBlob,xTarget,yTarget,sigmasqr)\n"
"\n"
"Parameters\n"
"----------\n"
...
static PyMethodDef _vv2par_C_cpu[] = {
   {"vv2par_C_cpu", vv2par_C_cpu, METH_VARARGS,vv2par_C_cpu_docs},
   {NULL} /*Sentiner - marks the end of this Python structure*/
};

/*--------------Initialize the vv2par functions -------------------------*/
void init_vv2par_C_cpu(){
   (void) Py_InitModule("_vv2par_C_cpu", _vv2par_C_cpu);
...
// Python wrapping function, pass data between Python and C and call CUDA Host Code.
    ...
    #define PRIVATE_VARS xk, yk, vxk, vyk, dx, dy, s, radius_squared

    #pragma omp parallel for shared(cvx,cvy,cxB,cyB,cwB,EPS, inv_2pi) private(m, PRIVATE_VARS)
    for(k=0; k<nTargets; k++){
        ...
        for(m=0; m<nBlobs; m++){
            dx = cxT[k]-cxB[m];
            dy = cyT[k]-cyB[m];
            radius_squared = dx*dx + dy*dy + sigmasqr;
            s = cwB[m]/radius_squared;
            vxk -= dy * s;
            vyk += dx * s;
        }
        cvx[k] = inv_2pi*vxk;
