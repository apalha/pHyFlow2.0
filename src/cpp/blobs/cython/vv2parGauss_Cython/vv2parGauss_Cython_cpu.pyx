...
def vv2parGauss_Cython_cpu(np.ndarray[DTYPE_t, ndim=2] blobs, DTYPE_t sigmasqr):
    ...
    with nogil:
        for k in prange(nBlobs):
            ...
            for m in range(nBlobs):
                ...
                radius_squared = radiusx*radiusx + radiusy*radiusy + EPS
                s = (blobs[m,2]/radius_squared)*(1.0 - exp(-radius_squared/sigmasqr))
