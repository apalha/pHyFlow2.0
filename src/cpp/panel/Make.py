...
kernelsPath = os.path.join(externCodePanelPath, 'kernels')
...
panelMethodFunctions = [{'name'         :'Vortex Panel Kernel', 
                         'source_type'  :'Cython',
...
for functions in panelMethodFunctions:
    # initialize print output
    titleString = functions['name'] + ' :: ' + functions['source_type'] + ' code'
    print '== ' + titleString + ' ' + '='*(66-len(titleString))

    # make panelfunctions

    # move to the path of the Makefile
    os.chdir(functions['source_path'])
    ...
    if buildFlag:
        ...
        if not os.path.isdir(functions['bin_path']):
            os.makedirs(functions['bin_path'])

        # copy to binary dir
        for binFile in functions['bin_files']:
            shutil.copy(os.path.join(os.curdir, binFile), functions['bin_path'])
